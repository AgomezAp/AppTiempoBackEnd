...
import {
  TokenFirmaMobiliario, ActaMobiliario, DetalleActaMobiliario,
  Mobiliario, MovimientoMobiliario
} from '../../models/inventario/consumibles';
import { enviarCorreoFirmaMobiliario, enviarActaMobiliarioFirmada } from '../../config/inventario-email';
...
export const enviarSolicitudFirmaMobiliario = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    await TokenFirmaMobiliario.create({
      token, actaMobiliarioId: acta.id, correoReceptor: acta.correoReceptor,
      estado: 'pendiente', fechaEnvio: new Date()
    }, { transaction });
    await acta.update({ estado: 'pendiente_firma' }, { transaction });
    const muebles = (acta as any).detalles?.map((d: any) => ({
      nombre: d.mobiliario?.nombre || 'Mueble', categoria: d.mobiliario?.categoria || '',
      cantidad: d.cantidad, unidad: d.mobiliario?.unidadMedida || 'und.'
    })) || [];
    await enviarCorreoFirmaMobiliario(acta.correoReceptor, acta.nombreReceptor, token, muebles, acta.observacionesEntrega);
...
export const obtenerActaMobiliarioPorToken = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const tokenFirma = await TokenFirmaMobiliario.findOne({
      where: { token },
      include: [{ model: ActaMobiliario, as: 'acta', include: [{ model: DetalleActaMobiliario, as: 'detalles', include: [{ model: Mobiliario, as: 'mobiliario', attributes: ['id', 'nombre', 'categoria', 'descripcion', 'unidadMedida'] }] }] }]
    });
    if (!tokenFirma) { res.status(404).json({ msg: 'Token invalido o no encontrado' }); return; }
    ...
    if (tokenFirma.estado === 'rechazado') { res.status(400).json({ msg: 'Este acta fue devuelta para correccion', motivo: tokenFirma.motivoRechazo }); return; }
    ...
    const datosActa = {
      ...
      muebles: acta?.detalles?.map((d: any) => ({
        nombre: d.mobiliario?.nombre, categoria: d.mobiliario?.categoria,
        descripcion: d.mobiliario?.descripcion, cantidad: d.cantidad,
        unidadMedida: d.mobiliario?.unidadMedida, condicion: d.condicionEntrega
      })) || []
    };
    res.json(datosActa);
  } catch (error) {
...
export const firmarActaMobiliarioConToken = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const tokenFirma = await TokenFirmaMobiliario.findOne({
      where: { token },
      include: [{ model: ActaMobiliario, as: 'acta', include: [{ model: DetalleActaMobiliario, as: 'detalles', include: [{ model: Mobiliario, as: 'mobiliario' }] }] }],
      transaction
    });
    if (!tokenFirma) { await transaction.rollback(); res.status(404).json({ msg: 'Token invalido' }); return; }
    if (tokenFirma.estado !== 'pendiente') { await transaction.rollback(); res.status(400).json({ msg: `Este token ya no es valido. Estado: ${tokenFirma.estado}` }); return; }
    const acta = (tokenFirma as any).acta;
    ...
    await ActaMobiliario.update({ firmaReceptor: firma, estado: 'activa', fechaFirma: ahora }, { where: { id: acta.id }, transaction });
    for (const detalle of acta.detalles) {
      ...
      await Mobiliario.update({ stockActual: nuevoStock }, { where: { id: detalle.mobiliarioId }, transaction });
      await MovimientoMobiliario.create({
        mobiliarioId: detalle.mobiliarioId, tipoMovimiento: 'firma_entrega' as any,
        cantidad: cantidadEntregada, stockAnterior, stockNuevo: nuevoStock, motivo: 'firma_entrega',
        ...
        actaEntregaId: acta.id, fecha: ahora
      }, { transaction });
    }
    await transaction.commit();
    try { const io = getIO(); io.to('actas-mobiliario').emit('actaMobiliario:signed', { actaId: acta.id, estado: 'activa' }); } catch (e) {}
    try {
      const muebles = acta.detalles?.map((d: any) => ({ nombre: d.mobiliario?.nombre || 'Mueble', categoria: d.mobiliario?.categoria || '', cantidad: d.cantidad })) || [];
    ...
    } catch (emailError) { console.error('Error enviando confirmacion por correo:', emailError); }
...
export const rechazarActaMobiliarioConToken = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!tokenFirma) { await transaction.rollback(); res.status(404).json({ msg: 'Token invalido' }); return; }
    if (tokenFirma.estado !== 'pendiente') { await transaction.rollback(); res.status(400).json({ msg: 'Este enlace ya no es valido' }); return; }
    const acta = (tokenFirma as any).acta;
    await tokenFirma.update({ estado: 'rechazado', motivoRechazo: motivo }, { transaction });
    await ActaMobiliario.update({ estado: 'rechazada', observacionesDevolucion: `Rechazada por el receptor: ${motivo}` }, { where: { id: acta.id }, transaction });
    await transaction.commit();
    try { const io = getIO(); io.to('actas-mobiliario').emit('actaMobiliario:rejected', { actaId: acta.id, motivo }); } catch (e) {}
...
export const reenviarCorreoFirmaMobiliario = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const acta = (tokenFirma as any).acta;
    const muebles = acta?.detalles?.map((d: any) => ({ nombre: d.mobiliario?.nombre || 'Mueble', categoria: d.mobiliario?.categoria || '', cantidad: d.cantidad, unidad: d.mobiliario?.unidadMedida || 'und.' })) || [];
    await enviarCorreoFirmaMobiliario(acta.correoReceptor, acta.nombreReceptor, tokenFirma.token, muebles, acta.observacionesEntrega);
...
export const obtenerEstadoFirmaMobiliario = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const tokens = await TokenFirmaMobiliario.findAll({ where: { actaMobiliarioId: Number(id) }, order: [['fechaEnvio', 'DESC']] });
