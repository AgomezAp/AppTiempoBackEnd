...
import { TipoInventario } from '../../models/inventario/consumibles';

export const obtenerTiposInventario = async (req: Request, res: Response) => {
  try {
    const tipos = await TipoInventario.findAll({ where: { activo: true }, order: [['orden', 'ASC']] });
    res.json(tipos);
  } catch (error) {
...
export const crearTipoInventario = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (existente) { res.status(400).json({ msg: 'Ya existe un tipo de inventario con ese código' }); return; }
    const nuevoTipo = await TipoInventario.create({
      nombre, codigo, descripcion, icono: icono || 'fa-box', color: color || '#6c757d',
      orden: orden || 99, activo: true
    });
    res.status(201).json({ msg: 'Tipo de inventario creado exitosamente', tipo: nuevoTipo });
  } catch (error) {
...
export const actualizarTipoInventario = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json({ msg: 'Tipo de inventario actualizado exitosamente', tipo });
  } catch (error) {
